...
public sealed class SqliteSessionsRepository : ISessionsRepository
{
    private readonly IIdentityConnectionFactory _connectionFactory;
    private readonly Dictionary<Guid, SqliteSession> _snapshots = [];

    public SqliteSessionsRepository(IIdentityConnectionFactory connectionFactory)
    {
        ...
        IIdentityTransactionScope? transaction = null)
    {
        ...
        try
        {
            ...
            _snapshots[session.SessionId] = SqliteSession.FromSession(session);
    ...
    public async Task<Result> Update(
        ...
        IIdentityTransactionScope? transaction = null)
    {
        if (!_snapshots.TryGetValue(session.SessionId, out SqliteSession? snapshot))
        {
