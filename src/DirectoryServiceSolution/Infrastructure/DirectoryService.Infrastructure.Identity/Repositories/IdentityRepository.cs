...
public sealed class IdentityRepository : IIdentityRepository
{
    ...
    private readonly Dictionary<Guid, UserSnapshot> _snapshots = [];

    public IdentityRepository(IIdentityConnectionFactory connectionFactory)
    {
        ...
        IIdentityTransactionScope? transaction = null)
    {
        ...
        try
        {
            ...
            _snapshots[user.Id] = UserSnapshot.FromUser(user);
    ...
    public async Task<Result> Update(
        ...
        IIdentityTransactionScope? transaction = null)
    {
        if (!_snapshots.TryGetValue(user.Id, out UserSnapshot? snapshot))
        {
