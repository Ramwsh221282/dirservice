...
internal sealed class SqliteIdentityTransactionScope : IIdentityTransactionScope, IInternalConnectionAccessor
{
    private readonly IDbConnection _connection;
    private bool _completed;
    private bool _disposed;

    private SqliteIdentityTransactionScope(IDbConnection connection)
    {
    ...
    public static async Task<SqliteIdentityTransactionScope> Begin(
        IIdentityConnectionFactory connectionFactory,
        CancellationToken ct = default)
    {
        IDbConnection connection = connectionFactory.Create();
        CommandDefinition beginCommand = new("BEGIN IMMEDIATE;", cancellationToken: ct);
        await connection.ExecuteAsync(beginCommand);

        return new SqliteIdentityTransactionScope(connection);
    }
